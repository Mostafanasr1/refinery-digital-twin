...
def test_optional_details_cover_every_hero_instance_without_new_asset_ids():
    ...
    # Stage C slice expands optional detail to every plant type, preserving IDs.
    assert set(manifest['types']) == {a['type'] for a in assets}
    ...
    for asset_id, asset in expected.items():
        ...
        assert parts and all(part['part'].startswith(('hero_', 'slice_')) for part in parts)
