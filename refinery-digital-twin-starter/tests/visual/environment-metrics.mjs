...
if (stage === 'C' || ['06', '05b', '08', '09'].includes(task)) cases.push({ look: 'photoreal-night', stress: 0 });
const results = [];
for (const { look, stress } of cases) {
  ...
  if (!setup.valid) {
    await writeFile(resolve(root, `docs/metrics/stage${stage}-task-${task}-invalid-setup.json`), JSON.stringify({ displayState, validity: setup }, null, 2));
  ...
  try {
    ...
    if (stage === 'C' || ['06', '05b', '08', '09'].includes(task)) run.url += '&animate=1';
    ...
    result.budgetPass = before || (stress ? result.frameTimeMedianMs <= 25 : result.drawCallsMean <= (look === 'engineering' ? 150 : 200) && result.frameTimeMedianMs <= (look === 'engineering' ? 1000 / 60 : 25) && result.frameTimeP95Ms <= (look === 'engineering' ? 20 : stage === 'C' && look === 'photoreal-night' ? 25 : 33) && (look !== 'engineering' || initialDownloadBytes <= 13200115));
...
const suffix = process.argv.find(arg => arg.startsWith('--suffix='))?.split('=')[1] ?? (process.argv.includes('--correction') ? 'night-correction' : 'rerun');
assert.match(suffix, /^[a-z0-9-]+$/);
await writeFile(resolve(output, `stage${stage}-task-${task}-${suffix}.json`), JSON.stringify({ source: before ? 'Approved Task 1 source with identical profiler only, no rendering change' : `Task ${task} candidate, uncapped frame-time protocol`, results }, null, 2) + '\n');
...
await writeFile(resolve(output, `stage${stage}-task-${task}-${suffix}.md`), text); console.log(text);
