...
const before = false;
const task = process.argv.find(arg => arg.startsWith('--task='))?.split('=')[1] ?? '03';
...
for (const { look, stress } of cases) {
  ...
  const setup = measurementValidity({ fpsMedian: 0, previousFps: 0, refreshHz, state: displayState });
  if (!setup.valid) {
  ...
  try {
    ...
    result.validity = measurementValidity({ fpsMedian: result.fpsMedian, previousFps: result.previousFps, refreshHz, state: displayState });
...
const suffix = 'rerun';
await writeFile(resolve(output, `stageB-task-${task}-${suffix}.json`), JSON.stringify({ source: before ? 'Approved Task 1 source with identical profiler only, no rendering change' : 'Task 3 candidate', results }, null, 2) + '\n');
const text = `# Task ${task} ${suffix}\n\n| Look | Stress assets | Draw calls | Triangles | FPS median | p95 ms | Initial bytes | Budget |\n|---|---:|---:|---:|---:|---:|---:|---|\n${results.map(r => `| ${r.look} | ${r.stressAssets || 'normal'} | ${r.drawCallsMean.toFixed(1)} | ${r.trianglesMean.toFixed(0)} | ${r.fpsMedian.toFixed(1)} | ${r.frameTimeP95Ms.toFixed(2)} | ${r.initialDownloadBytes} | ${before ? 'baseline' : !r.validity.valid ? 'INVALID' : r.budgetPass ? 'PASS' : 'FAIL'} |`).join('\n')}\n\n` + results.map(r => `## ${r.look} / ${r.stressAssets || 'normal'} submissions\n\nDisplay: ${r.refreshHz} Hz; AC: ${r.displayState.acConnected}; laptop only: ${r.displayState.noExternalMonitor}; ${r.displayState.powerPlan}. Previous comparable FPS: ${r.previousFps.toFixed(1)}. Measurement: ${r.validity.valid ? 'VALID' : 'INVALID — ' + r.validity.reasons.join('; ')}.\n\n| Category | Color | Shadow |\n|---|---:|---:|\n${Object.keys(r.categoryMean.color).map(name => `| ${name} | ${r.categoryMean.color[name].toFixed(1)} | ${r.categoryMean.shadow[name].toFixed(1)} |`).join('\n')}\n\nPost passes: ${r.categoryMean.post.toFixed(1)}.\n`).join('\n');
