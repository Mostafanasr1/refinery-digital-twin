...
const photoLook = process.argv.includes('--night') ? 'photoreal-night' : 'photoreal';
...
try {
  ...
  await switchLook(run.page, photoLook);
  ...
  for (const geometry of ['blender', 'proxy']) {
    ...
    // Warm at the measured camera after geometry replacement: newly visible
    // effect meshes can upload for the first time outside the startup frustum.
    const coldGeometryCount = (await snapshot(run.page)).geometries;
    await switchLook(run.page, photoLook);
    ...
    for (let index = 0; index < 10; index++) {
      const look = index % 2 ? 'engineering' : photoLook;
    ...
    results.push({ geometry, coldGeometryCount, initialGeometryCount: before.geometries, finalGeometryCount: after.geometries, counts, lookSwitches: 10, glbRequestsDuringSwitches: glbRequests - requestsBefore, selection: 'PASS', layers: 'PASS', process: 'PASS', scenario: 'PASS', camera: 'PASS' });
  }
  // Fresh direct URL proves persistence, and supplies all fixed photoreal captures.
  await openLook(run, photoLook);
  ...
  const report = { hardware, results, directPhotorealUrl: 'PASS', screenshotCount: config.cameras.length * 2, errors: run.errors };
