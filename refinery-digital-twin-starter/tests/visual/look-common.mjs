...
export async function openLook(run, look = 'engineering') {
  ...
  if (look === 'photoreal') await run.page.waitForFunction(() => document.querySelector('canvas')?.dataset.environmentReady === 'true' && document.querySelector('canvas')?.dataset.materialsReady === 'true' && document.querySelector('canvas')?.dataset.detailReady === 'true');
...
export async function switchLook(page, look) {
  ...
  if (look === 'photoreal') await page.waitForFunction(() => document.querySelector('canvas')?.dataset.environmentReady === 'true' && document.querySelector('canvas')?.dataset.materialsReady === 'true' && document.querySelector('canvas')?.dataset.detailReady === 'true');
