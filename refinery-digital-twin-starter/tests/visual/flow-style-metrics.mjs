...
const output=resolve(root,'docs/metrics/stageC-task-04-correction.json');
const quantile=(v,q)=>v.toSorted((a,b)=>a-b)[Math.ceil(v.length*q)-1];
for(const style of ['pipes','arcs'])for(const look of ['engineering','photoreal','photoreal-night'])for(const index of look==='photoreal-night'?[1,5]:[5])for(const path of [paths[0]]){
 ...
 const run=await openRun({metrics:true});run.url+=`&animate=1&captureCycle=1&flow=${style}`;
 try{
  ...
  const result={style,look,camera:config.cameras[index].id,path:path.process_path_id,hardware,displayState,validity,medianMs,p95Ms,callsMax,callsMean:samples.reduce((n,s)=>n+s.calls,0)/samples.length,derivedFps:1000/medianMs,pass,samples};results.push(result);
