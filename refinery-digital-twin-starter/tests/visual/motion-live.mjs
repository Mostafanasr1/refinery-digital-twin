...
const output = resolve(root, 'docs/handbacks/evidence/stageC-task-01/live');
...
for (const [name, executable] of [['chrome', 'C:/Program Files/Google/Chrome/Application/chrome.exe'], ['edge', 'C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe']]) {
  ...
  try {
    ...
    const record = { status: 'PASS', browser: name, hardware, release, preview, checks: ['release unchanged', 'preview motion advances', 'slider keyboard and presets', 'master freeze', 'day/night and emulated mobile captures'], errors: run.errors, phone: 'Physical Samsung A35 review pending Mostafa' };
