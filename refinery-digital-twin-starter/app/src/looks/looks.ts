export type LookId = 'engineering' | 'photoreal' | 'photoreal-night';
...
export interface Look {
  ...
  post: { toneMapping: 'aces' | 'agx'; ao?: boolean; exposure: number; bloom: { intensity: number; radius: number; threshold: number } };
}
...
const dayLooks = {
  ...
  photoreal: { ...engineering, id: 'photoreal', label: 'Photoreal', detailLevel: 1,
    ...
    post: { toneMapping: 'aces', ao: true, exposure: .85, bloom: { intensity: .18, radius: .35, threshold: 1.8 } },
  },
} satisfies Record<'engineering' | 'photoreal', Look>;
const photoreal: Look = dayLooks.photoreal;
...
export function lookFromUrl(url: URL): LookId { return url.searchParams.get('look') === 'photoreal-night' ? 'photoreal-night' : url.searchParams.get('look') === 'photoreal' || (!url.searchParams.has('look') && url.hash === '#preview') ? 'photoreal' : 'engineering'; }
