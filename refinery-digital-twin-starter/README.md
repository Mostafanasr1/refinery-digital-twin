...
Canonical plant truth is frozen and verified by `data:verify`. Presentation cameras, looks and tours live separately under `data/presentation` and application look configuration. Original project geometry is procedural; externally sourced runtime art is registered with its verified CC0 or CC BY 4.0 license. User reference images and review videos are not runtime assets. See [ASSETS.md](ASSETS.md), [repo facts](docs/REPO_FACTS.md), [approved budgets](docs/APPROVED_BUDGETS.md) and [Task 9 hand-back](docs/handbacks/stageB-task-09.md).
...
The `/next/` candidate replaces generated ridges with adapted SRTM terrain from Sinai and blends Poly Haven sand, gravel and rock. Instanced sourced cabins, containers and vehicles remain presentation-only under **Dressing**. The Task 3 correction adds plant dressing to engineering in flat grey; rocks, scrub and landscape stay photoreal only. Canonical equipment remains unchanged. Sources, licenses and derivative transformations are listed in [ASSETS.md](ASSETS.md); see the [Task 3 hand-back](docs/handbacks/stageC-task-03.md) and [CAM-6 comparison](docs/handbacks/evidence/stageC-task-03/comparison.html). Task 3 plant detail and night lighting were approved; terrain is accepted for now, with far hills and slope tiling deferred. Both reviewers approved the correction at the combined Task 4 gate; its engineering candidate was promoted before the Task 4 candidate. No reference-level realism claim.

### Stage C Task 4 — pipe-following process flow

Process paths in both looks use instanced gradient tubes along connecting pipes, with a subtle tint on active pipe segments. Pause, Motion, pump-trip recovery and the guided tour control the same route. Both reviewers approved the Task 3 correction and Task 4 engineering candidates; they were promoted in that order. See the [combined hand-back](docs/handbacks/stageC-task-04.md) and [review gallery](docs/handbacks/evidence/stageC-task-04/review.html). The Stage C release includes both approved tasks. `/next/` remains available for continued work.

Choose a Process path to expose the adjacent **Flow** control: **Pipes** follows physical pipes; **Arcs** uses raised connecting tubes. Both have travelling gradients, remain visible at wide framing and work in either look. The choice persists in the URL (`flow=arcs`; Pipes is the default) and does not interrupt a guided tour. See the [closing correction and release record](docs/handbacks/stageC-task-04-correction.md).
